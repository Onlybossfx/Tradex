/*
  ============================================================
  dashboard.backend.js — TraydR SHARED BACKEND v2
...
window.DB = {
  ...
  logout: async () => { await _dbSb.auth.signOut(); window.location.href = 'traydr-auth.html'; },
  updateProfile: async (meta) => {
    const { error } = await _dbSb.auth.updateUser({ data: meta });
    if (!error) {
      const { data: { user } } = await _dbSb.auth.getUser();
      await _dbSb.from('users').update({
  ...
  changePassword: async (newPassword) => {
    const { error } = await _dbSb.auth.updateUser({ password: newPassword });
    return { error };
  },
  getUserProfile: async (userId) => {
    const { data, error } = await _dbSb.from('users').select('*').eq('id', userId).single();
    return { data, error };
  },

  /* ══ LISTINGS ══ */
  getMyListings: async (userId) => {
    const { data, error } = await _dbSb.from('listings').select('*')
      .eq('seller_id', userId).order('created_at', { ascending: false });
    return { data: data || [], error };
  },
  createListing: async (payload) => {
    const { data, error } = await _dbSb.from('listings').insert(payload).select().single();
    return { data, error };
  },
  updateListing: async (id, userId, payload) => {
    const { data, error } = await _dbSb.from('listings').update(payload)
      .eq('id', id).eq('seller_id', userId).select().single();
    return { data, error };
  },
  deleteListing: async (id, userId) => {
    const { error } = await _dbSb.from('listings').delete().eq('id', id).eq('seller_id', userId);
    return { error };
  },
  ...
  updateOrderStatus: async (orderId, status, userId, role = 'seller') => {
    ...
    const { data: order } = await _dbSb.from('orders').select('*').eq('id', orderId).single();
    const { error } = await _dbSb.from('orders').update({ status }).eq('id', orderId).eq(col, userId);
    if (!error && order) {
  ...
  getSavedListings: async (userId) => {
    const { data, error } = await _dbSb.from('saved_listings').select('listing_id').eq('user_id', userId);
    if (error || !data?.length) return { data: [], error };
    const ids = data.map(r => r.listing_id);
    const { data: listings } = await _dbSb.from('listings').select('*').in('id', ids).eq('status', 'active');
    return { data: listings || [], error: null };
  },
  unsaveListing: async (userId, listingId) => {
    const { error } = await _dbSb.from('saved_listings').delete().eq('user_id', userId).eq('listing_id', listingId);
    return { error };
  },
  ...
  getConversations: async (userId) => {
    ...
    const convs = await Promise.all((orders || []).map(async (o) => {
      const { data: msgs } = await _dbSb.from('messages').select('content, created_at, sender_id, read')
        .eq('order_id', o.id).order('created_at', { ascending: false }).limit(1);
      const { count: unread } = await _dbSb.from('messages').select('*', { count: 'exact', head: true })
  ...
  sendMessage: async (orderId, senderId, receiverId, content) => {
    const { data, error } = await _dbSb.from('messages')
  ...
  submitReview: async (orderId, listingId, reviewerId, revieweeId, rating, comment) => {
    const { data, error } = await _dbSb.from('reviews')
  ...
  getListingReviews: async (listingId) => {
    const { data, error } = await _dbSb.from('reviews').select('*, users(full_name, avatar_url)')
  ...
  getPayouts: async (userId) => {
    const { data, error } = await _dbSb.from('payouts').select('*')
      .eq('seller_id', userId).order('created_at', { ascending: false });
    return { data: data||[], error };
  },
  requestPayout: async (userId, amount, method, account) => {
    const { data, error } = await _dbSb.from('payouts')
  ...
  getPayoutMethod: async (userId) => {
    const { data } = await _dbSb.from('users').select('payout_method, payout_account').eq('id', userId).single();
    return data || {};
  },

  /* ══ NOTIFICATIONS ══ */
  getNotifications: async (userId, limit=30) => {
    const { data, error } = await _dbSb.from('notifications').select('*')
      .eq('user_id', userId).order('created_at',{ascending:false}).limit(limit);
    return { data: data||[], error };
  },
  markNotifRead:    async (id) => { await _dbSb.from('notifications').update({read:true}).eq('id',id); },
  markAllNotifsRead:async (userId) => { await _dbSb.from('notifications').update({read:true}).eq('user_id',userId).eq('read',false); },
  getUnreadNotifCount: async (userId) => {
    const {count} = await _dbSb.from('notifications').select('*',{count:'exact',head:true}).eq('user_id',userId).eq('read',false);
    return count||0;
  },
  createNotification: async (userId, type, title, body, link=null) => {
    if (!userId) return { error: null };
    const {error} = await _dbSb.from('notifications').insert({user_id:userId,type,title,body,link});
    return {error};
  },
  subscribeToNotifications: (userId, callback) => {
    return _dbSb.channel(`notifs:${userId}`)
  ...
  getListingAnalytics: async (userId) => {
    const { data: listings, error } = await _dbSb.from('listings')
    ...
    if (error) return { data:null, error };
    const { data: views } = await _dbSb.from('listing_views').select('listing_id, created_at')
  ...
  logListingView: async (listingId, userId=null) => {
    await _dbSb.rpc('increment_listing_view',{listing_id_arg:listingId});
    await _dbSb.from('listing_views').insert({listing_id:listingId,viewer_id:userId}).select();
  },

  /* ══ ADDRESSES ══ */
  getAddresses: async (userId) => {
    const {data,error}=await _dbSb.from('addresses').select('*').eq('user_id',userId).order('is_default',{ascending:false});
    return {data:data||[],error};
  },
  saveAddress: async (userId, address) => {
    if (address.is_default) await _dbSb.from('addresses').update({is_default:false}).eq('user_id',userId);
    if (address.id) {
      const {id,...rest}=address;
      const {data,error}=await _dbSb.from('addresses').update(rest).eq('id',id).eq('user_id',userId).select().single();
      return {data,error};
    }
    const {data,error}=await _dbSb.from('addresses').insert({...address,user_id:userId}).select().single();
    return {data,error};
  },
  deleteAddress: async (id, userId) => {
    const {error}=await _dbSb.from('addresses').delete().eq('id',id).eq('user_id',userId);
    return {error};
  },
  ...
  getUnreadMessageCount: async (userId) => {
    const {count}=await _dbSb.from('messages').select('*',{count:'exact',head:true}).eq('receiver_id',userId).eq('read',false);
    return count||0;
  },
  getPendingOrderCount: async (userId, role='seller') => {
    const col=role==='seller'?'seller_id':'buyer_id';
    const {count}=await _dbSb.from('orders').select('*',{count:'exact',head:true}).eq(col,userId).in('status',['pending','escrowed','delivered']);
    return count||0;
  },
  ...
  admin: {
    isAdmin: async () => {
      ...
      const {data}=await _dbSb.from('users').select('is_admin').eq('id',user.id).single();
      return data?.is_admin===true;
    },
    getStats: async () => {
      const [users,listings,orders,disputes]=await Promise.all([
        _dbSb.from('users').select('*',{count:'exact',head:true}),
        _dbSb.from('listings').select('*',{count:'exact',head:true}).eq('status','active'),
        _dbSb.from('orders').select('*',{count:'exact',head:true}),
        _dbSb.from('disputes').select('*',{count:'exact',head:true}).eq('status','open'),
      ]);
    ...
    getUsers: async ({search='',role='',page=1,perPage=20}={}) => {
      let q=_dbSb.from('users').select('*',{count:'exact'});
    ...
    getListings: async ({search='',category='',status='',page=1,perPage=20}={}) => {
      let q=_dbSb.from('listings').select('*',{count:'exact'});
    ...
    getOrders: async ({status='',page=1,perPage=20}={}) => {
      let q=_dbSb.from('orders').select('*',{count:'exact'});
      if(status)q=q.eq('status',status);
      const from=(page-1)*perPage;
      q=q.range(from,from+perPage-1).order('created_at',{ascending:false});
      const {data,count,error}=await q;
    ...
    getDisputes: async ({status=''}={}) => {
      let q=_dbSb.from('disputes').select('*, orders(listing_title,amount), buyer:buyer_id(full_name,email), seller:seller_id(full_name,email)');
      if(status)q=q.eq('status',status);
    ...
    resolveDispute: async (id, resolution, winner) => {
      ...
      const {data:dispute}=await _dbSb.from('disputes')
    ...
    getPayouts: async ({status=''}={}) => {
      let q=_dbSb.from('payouts').select('*, seller:seller_id(full_name,email)');
      if(status)q=q.eq('status',status);
      const {data,error}=await q.order('created_at',{ascending:false});
      return {data:data||[],error};
    },
    processPayout: async (payoutId, status, reference='') => {
      const {data:payout}=await _dbSb.from('payouts').select('*, seller:seller_id(email,full_name)').eq('id',payoutId).single();
      const {error}=await _dbSb.from('payouts').update({
