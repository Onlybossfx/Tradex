/*
  ============================================================
  auth.backend.js — TraydR AUTH BACKEND
  ...
  PAGES THAT USE THIS FILE:
    traydr-auth.html      → login, signup, OAuth
...
window.Auth = {
  ...
  /* ─────────────────────────────────────────────
     SIGNUP
     Called by: traydr-auth.html (step 2)
  ...
  signup: async (email, password, meta) => {
    const { data, error } = await sb.auth.signUp({
      ...
      options: {
        data: meta,
        emailRedirectTo: `${BASE_URL}/traydr-auth.html?confirmed=1`,
      },
    });
  ...
  loginWithOAuth: async (provider) => {
    await sb.auth.signInWithOAuth({
      provider,
      options: {
        redirectTo: `${BASE_URL}/traydr-auth.html?oauth=1`,
      },
    });
  },
  ...
  onSuccess: async ({ context, role }) => {
    ...
    if (returnTo) {
      sessionStorage.removeItem('traydr_return_to');
      window.location.href = returnTo;
      return;
    }

    /* Also handle legacy checkout redirect */
    if (sessionStorage.getItem('traydr_checkout') && context === 'login') {
